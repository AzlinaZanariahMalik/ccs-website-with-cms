@extends('back.layouts.pages-layout')
@section('title') {{'Organization'}} @endsection

@section('content')


<!-- sidebar menu admin -->
<section id="sidebar">
		<a href="{{ route('admin.home')}}" class="logo-sidebar"> <img src="{{ url('/images/CCS_logo.png') }}" width="53" height="53" class="d-inline-block align-top" alt=""> &nbsp&nbsp Content Management</a>
		<ul class="side-menu">
			<li><a href="{{ route('admin.home')}}"><img class="icon" src="{{'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABgAAAAYCAYAAADgdz34AAAAAXNSR0IArs4c6QAAAHlJREFUSEtjZKAxYKSx+Qx0t2A/AwODAw5fPWBgYFCEyhGrDsMH/wkEGczHxKobtQBngMITD3oqIjZsiVVH/ziYz8DAkIDD4wcYGBgcoXLEqqN/RqN6yYEeycTmUGLV0T+SiU1+xKob9cEgLCqIzaHEqhuGOZnqRQUAZX0oGXY3cFsAAAAASUVORK5CYII='}}"/>&nbsp Dashboard</a></li>
			@if(auth()->user()->user_type == 1)
			<li><a href="{{ route('admin.user-manage')}}" class=""><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Users</a></li>
			<li><a href="{{ route('admin.files-management')}}" class=""><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Files</a></li>
            <li><a href="{{ route('admin.announcement')}}" ><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Announement</a></li>
			<li class="divider" data-text="Content Manage">Content Manage</li>
			<li>
				<a href="" ><img class="icon" src="{{ 'data:image/png;base64,' }}"/>&nbsp About College 
                <img class="icon-right" src="{{'data:image/png;base64,'}}"/></a>
				<ul class="side-dropdown">
                    <li><a href="{{ route('admin.about')}}">About</a></li>
					<li><a href="{{ route('admin.vision')}}">Vision</a></li>
					<li><a href="{{ route('admin.mission')}}">Mission</a></li>
                    <li><a href="{{ route('admin.goal')}}">College Goal</a></li>
					<li><a href="{{ route('admin.college-history')}}">History</a></li>
					<li><a href="{{ route('admin.department')}}">Department</a></li>
					<li><a href="{{ route('admin.faculty-and-staff')}}">Faculty and Staff</a></li>
					
				</ul>
			</li>
            
			<li><a href="{{ route('admin.academic-program')}}"><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Academic</a></li>
			<li><a href="{{ route('admin.banner')}}"><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Banner</a></li>
			@endif
			@if(auth()->user()->rank == 1)
			<li><a href="{{ route('admin.deans-corner')}}"><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Dean's Corner</a></li>
			@endif
			<li><a href="{{ route('admin.news.news-management')}}" ><img class="icon" src="{{'data:image/png;base64,'}}"/></i>&nbsp News</a></li>
			@if(auth()->user()->user_type == 1)
            <li><a href="{{ route('admin.pythons')}}"class="active" ><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Pythons</a></li>
            <li><a href=""><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Alumni</a></li>
			<li><a href="{{ route('admin.contact')}}"><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Contact</a></li>
			<li><a href="{{ route('admin.certifications')}}"><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Certification</a></li>
			<li class="divider" data-text="Maintenance">Maintenance</li>
			
            <li><a href="{{ route('admin.settings')}}"><img class="icon" src="{{'data:image/png;base64,'}}"/></i>&nbsp Settings</a></li>
            @endif
			<li><a href="{{ route('admin.profile')}}" ><img class="icon" src="{{'data:image/png;base64,'}}"/>&nbsp Profile</a></li>
			
		</ul>
		
	</section>
	<!-- end of sidebar menu -->
	<!-- page body content -->
	<section id="content">
		<!-- header-->
		@livewire('nav-header')
		<!-- end of header of main content -->

		<!-- main content -->
		<main>
			<h1 class="title">Organization</h1>
			<ul class="breadcrumbs">
				<li><a href="{{ route('admin.home')}}">Home</a></li> / 
				<li><a href="{{ route('admin.pythons')}}" >Organization</a></li> /
				<li><a href="#" class="active">Edit Organization</a></li>
			
			</ul>
		
        
       @livewire('organization.edit-form',['id' => Route::current()->parameter('id')])
        

      

			
		</main>
		<!-- end of main content -->
		<!--footer section-->
		<div class="footer text-center">
			<span >© 2023 Constellation WebTech </span>
		</div>
	</section>
	<!-- end of body of page content -->

@endsection